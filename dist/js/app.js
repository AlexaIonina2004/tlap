 ...
 */            (function(global, factory) {
    ...
    (() => {
        ...
        let script_parent = document.querySelectorAll(".down-menu__link");
        let child = document.querySelectorAll(".down-menu__info");
        let nowI = 0;
        child[nowI]?.classList.remove("close");
        for (let i = 0; i < script_parent.length; i++) for (let j = 0; j < child.length; j++) if (i == j) {
            script_parent[i].onmouseover = function(event) {
                child[nowI].classList.toggle("close");
                child[i].classList.remove("close");
            };
            script_parent[i].onmouseout = function(event) {
                nowI = i;
            };
        }
        let calcs = [];
        let indicators = [];
        let other = [];
        script_parent?.forEach(((n, i) => {
            n.textContent.includes("Калькулятор") || n.textContent.includes("Расчет сложного процента") ? calcs.push(n) : n.textContent.includes("Индикатор") || n.textContent.includes("Корреляция валютных пар") || n.textContent.includes("Стакан ордеров Forex") || n.textContent.includes("Отчеты Commitments of Traders") ? indicators.push(n) : other.push(n);
        }));
        ...
        let calc = calcs.map((name => `<li class="under-menu__item"><a href="#" class="under-menu__link ">${name.textContent}</a></li>`)).join("");
        let otherItems = other.map((name => `<li class="under-menu__item"><a href="#" class="under-menu__link ">${name.textContent}</a></li>`)).join("");
